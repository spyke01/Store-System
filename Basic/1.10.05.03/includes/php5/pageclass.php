...
class pageClass { 
	...
	var $topmenu = array();
	var $productCategoriesLeftMenu = array();
	var $userOptionsLeftMenu = array();
	var $adminOptionsLeftMenu = array();
	var $breadCrumbTrail = array();
	...
	function makeMenuItem($menu, $label, $page, $class = "") {
		...
		if ($menu == "productCategoriesLeft") {
			$this->productCategoriesLeftMenu[$label] = array();
			$this->productCategoriesLeftMenu[$label]['value'] = $this->makeNavURL($page);
			if ($class != "") { $this->productCategoriesLeftMenu[$label]['class'] = $class; }
		}
	...
	function printSidebar($tagId, $tagClass = "") {		
		...
		if ($this->templateVars['pCLm_active'] == ACTIVE) {
			$sidebarHTML .= "
					<li class=\"title\">Product Categories</li>";
			
			foreach ($this->productCategoriesLeftMenu as $text => $page) {
				$sidebarHTML .= "
					<li><a href=\"" . $this->productCategoriesLeftMenu[$text]['value'] . "\"><span>" . $text . "</span></a></li>";
			}
		}
