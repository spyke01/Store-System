...
function ajaxDeleteProductNotifier(productid, orderid, action, text, row) {
    if (confirm("Are you sure you want to delete this "+ text +"?")) {
		sqr_show_hide(productid + 'ProductSpinner');
		...
		new Ajax.Request(action, {asynchronous:true, onSuccess:function(){ 
			new Effect.SlideUp(row);
		
			new Ajax.Request('ajax.php?action=reCalculateOrderPrices&id=' + orderid, {asynchronous:true, onSuccess:function(){ 
				sqr_show_hide(orderid + 'taxSpinner');
...
function ajaxUpdateOrderTotals(orderid, productid) {
	sqr_show_hide(productid + 'itemsSpinner');
	new Ajax.Request('ajax.php?action=reCalculateOrderPrices&id=' + orderid, {asynchronous:true, onSuccess:function(){ 
		new Ajax.Updater(productid + 'itemsContainer', 'ajax.php?action=getitem&table=orders_products&item=total_cost&id=' + productid, {asynchronous:true, onSuccess:function(){ sqr_show_hide(productid + 'itemsSpinner'); }});
...
function ajaxCalculateShippingInfo(orderid, zipCode, shippingChoiceID) {
	new Ajax.Request('ajax.php?action=calculateShippingFromZipCode&zipCode=' + zipCode, {asynchronous:true, onSuccess:function(){ ajaxUpdateShippingInfo(orderid, shippingChoiceID); }});
}
...
/*-------------------------------------------------------------------------*/
// ProductFeatures Functions
/*-------------------------------------------------------------------------*/	
productFeaturesCounter = 0;
productFeatureSetAddImageSrc = "";
productFeatureSetDeleteImageSrc = "";

function configureProductFeaturesSettings(count, addImageSrc, deleteImageSrc) {
	productFeaturesCounter = count;
	productFeatureSetAddImageSrc = addImageSrc;
	productFeatureSetDeleteImageSrc = deleteImageSrc;
}
...
function ajaxDeleteProductFeatureSet(elementId) {
    if (confirm("Are you sure you want to delete this Product Feature?")) {
		removeElement(elementId);
	}
	checkForNoProductFeatures();
}

function checkForNoProductFeatures() {
	var p = document.getElementById('productFeatures');
    if (p.innerHTML == "") {
		p.innerHTML = "You deleted all the ProductFeatures, <a href=\"\" onclick=\"javascript:clearProductFeaturesAndAddProductFeatureSet()(); return false;\">click here to add a new one</a>.";
	}
}

function clearProductFeaturesAndAddProductFeatureSet() {
	document.getElementById('productFeatures').innerHTML == "";
	addProductFeatureSet('', '');
}
			
function addProductFeatureSet(nameValue, priceValue) {
	productFeaturesCounter++; // increment productFeaturesCounter to get a unique ID for the new element
	
	// Create our TR
	addElement('productFeatures', 'tr', 'productFeatureSetRow' + productFeaturesCounter, '');
	
	// Build our code for our items
	var productFeatureSetNameTDContents = "<input type=\"text\" name=\"productFeatureSetName[]\" size=\"60\" value=\"" + nameValue + "\" />";
	var productFeatureSetPriceTDContents = "<input type=\"text\" name=\"productFeatureSetPrice[]\" size=\"10\" value=\"" + priceValue + "\" /><a href=\"\" onclick=\"javascript:addProductFeatureSet('', ''); return false;\"><img src=\"" + productFeatureSetAddImageSrc + "\" alt=\"Add New ProductFeature\" /></a><a href=\"\" onclick=\"javascript:ajaxDeleteProductFeatureSet('productFeatureSetRow" + productFeaturesCounter + "'); return false;\"><img src=\"" + productFeatureSetDeleteImageSrc + "\" alt=\"Remove This ProductFeature\" /></a>";
	
	// Add the contents to the table
	addElement('productFeatureSetRow' + productFeaturesCounter, 'td', '', productFeatureSetNameTDContents);
	addElement('productFeatureSetRow' + productFeaturesCounter, 'td', '', productFeatureSetPriceTDContents);
}
